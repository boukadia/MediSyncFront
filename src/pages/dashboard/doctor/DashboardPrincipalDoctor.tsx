...
function DashboardPrincipalDoctor() {
  ...
  const todayAppointment = myAppointments.filter((appointment) => {
    if (!appointment.date) return false;
    const appointmentDate = new Date(appointment.date);
    if (isNaN(appointmentDate.getTime())) return false;
    return appointmentDate.toISOString().split('T')[0] === new Date().toISOString().split('T')[0];
  });
    ...
    <div className="doctor-dashboard">
      ...
      <div className="container-fluid px-4 py-3">
        ...
        <div className="d-flex justify-content-between align-items-center mb-4 page-header">
          <div>
            <h1 className="h3 mb-1">Bonjour, {getDoctor()?.name} </h1>
            <div className="d-flex gap-2 flex-wrap">
              <span className="badge badge-info">
                <i className="fas fa-shield-heart me-1"></i>Données chiffrées
              </span>
              <span className="badge badge-success">
                <i className="fas fa-check me-1"></i>{activePatients} patients actifs
              </span>
            </div>
          </div>
        </div>
          ...
          <div className="col-lg-3 col-md-6">
            <div className="card stat-card h-100">
              <div className="card-body d-flex align-items-center">
                ...
                <div className="ms-3">
                  <div className="stat-value">{filterTodayAppointments.length}</div>
